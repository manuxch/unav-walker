...
int main(int argc, char *argv[]) {
  ...
  cout << "# walkers-forceWall ver. 1.1" << endl;
  cout << "# 2026.06.29" << endl;
  ...
  // Contenedor: cadena de 4 vértices (U sin fondo, de (-R,0) a (R,0))
  // El fondo de medición se agrega como cuerpo separado (gID=-200).
  b2BodyDef bd;
  ...
  wall_poly[1].Set(-gs->silo.R, gs->silo.H);
  wall_poly[2].Set(gs->silo.R, gs->silo.H);
  wall_poly[3].Set(gs->silo.R, 0.0f);

  b2ChainShape siloShape_poly;
  siloShape_poly.CreateChain(wall_poly, 4, wall_poly[0], wall_poly[3]);
  ...
  b2Vec2 wall_poly_2[4];
  for (int i = 0; i < 4; ++i)
    wall_poly_2[i] = wall_poly[3 - i];
  b2ChainShape siloShape_poly_2;
  siloShape_poly_2.CreateChain(wall_poly_2, 4, wall_poly_2[0], wall_poly_2[3]);
  ...
  cout << "#\t- Silo creado (3 lados, sin fondo)." << endl;

  // Fondo de medición: borde horizontal de ancho completo (-R a R), gID=-200.
  // Nunca se remueve; todos los contactos con este cuerpo son fuerzas sobre el fondo.
  b2BodyDef fondo_bd;
  fondo_bd.position.Set(0.0f, 0.0f);
  fondo_bd.type = b2_staticBody;
  BodyData *fondoD = new BodyData;
  fondoD->isGrain = false;
  fondoD->nLados = 2;
  fondoD->gID = -200;
  fondo_bd.userData.pointer = uintptr_t(fondoD);
  b2Body *fondo = world->CreateBody(&fondo_bd);
  b2Vec2 fv1(-gs->silo.R, 0.0f);
  b2Vec2 fv2(gs->silo.R, 0.0f);
  b2EdgeShape fondo_shape;
  fondo_shape.SetTwoSided(fv1, fv2);
  b2FixtureDef fondo_fix;
  fondo_fix.shape = &fondo_shape;
  fondo_fix.density = 0.0f;
  fondo_fix.friction = gs->silo.fric;
  fondo->CreateFixture(&fondo_fix);
  cout << "#\t- Fondo de medición creado (gID=-200, x: ["
       << -gs->silo.R << ", " << gs->silo.R << "])." << endl;
  ...
  for (int i = 0; i < gs->noTipoGranos; i++) {
    ...
    for (int j = 0; j < gs->granos[i]->noGranos; j++) {
  ...
  size_t bin_count[gs->n_bin_perfiles]{0};

  // Archivo de fuerzas sobre el fondo (gID=-200)
  ...
  // Fase de deposición (equivalente al tBlock en silo-vib)
  ...
  while (t < gs->tBlock) {
    ...
    world->Step(tStep, pIter, vIter);
    // Fuerza sobre el fondo de medición (gID=-200)
  ...
  cout << "# Fin de la fase de deposición." << endl;

  // Bucle principal de simulación (fondo permanece cerrado)
  size_t n_reg = 0;
  double p_min = 1.0e8;
  double p_max = -1.0e8;
  cout << "# Inicio de la simulación con medición de fuerzas ..." << endl;
  while (t < gs->maxT) {
    auto [bpos, bvel, bac] = exitacion_mm(t, gamma, w, gs);
    do_base_force(world, bvel, epsilon_v, gs->g);
    do_rot_friction(world, gs);
