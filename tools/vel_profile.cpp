 ...
 *
 * Opciones:
 *   --qty <nombre...>  Cantidades a promediar (default: vy)
 *                      Opciones: vy  vx  w  speed  Eklin  Ekrot
...
enum class Qty { VY, VX, W, SPEED, EKLIN, EKROT };
...
static double extract_qty(const dem::VelocityEntry& p, Qty q)
{
    switch (q) {
    case Qty::VY:    return p.vy;
    case Qty::VX:    return p.vx;
    case Qty::W:     return p.w;
    case Qty::SPEED: return std::hypot(p.vx, p.vy);
    case Qty::EKLIN: return p.E_kin_lin;
    case Qty::EKROT: return p.E_kin_rot;
    }
    return 0.0;
}
...
struct BinAccum {
    ...
    std::vector<size_t> count; ///< Número de partículas acumuladas por bin
    size_t n_bins;
...
struct FrameTask {
    ...
    void operator()() const
    {
        ...
        for (const auto& p : frame.particles) {
            // Filtro en x
            if (std::abs(p.x) > x_m) continue;

            // Calcular bin en y
            double rel = p.y - y_min;
            if (rel < 0.0) continue;
            ...
            for (size_t qi = 0; qi < qtys.size(); ++qi)
                local[qi].add(ibin, extract_qty(p, qtys[qi]));
        }
...
static void print_usage(const char* prog)
{
    std::cout <<
        ...
        "Opciones:\n"
        "  --qty <qty...>  Cantidades: vy vx w speed Eklin Ekrot  (default: vy)\n"
        "  --threads N     Número de worker threads (default: hardware_concurrency)\n\n"
